...
M5~M7을 통해 진단이 더 정밀해졌다. 병목이 두 개이며 직렬로 연결되어 있다.

### 병목 1: z가 action에 blind하다 (detach 문제) ← 더 근본적

z가 어떤 행동을 해야 하는지 정보를 전혀 받지 못하는 상태 — 이걸 "action-blind"라고 한다.

`compute_loss()`에서 action flow loss와 prior flow loss 모두 `z_star.detach()`를 사용한다. posterior 인코더가 받는 gradient는 semantic loss(미래 이미지 cosine sim)와 InfoNCE뿐이다.

결과: z는 "미래를 닮은 z"와 "task를 구분하는 z"가 되도록만 학습되고, **"action을 잘 예측하는 z"가 되도록 강제되지 않는다.** M2(oracle, posterior MSE=0.0017)와 M7(posterior MSE=0.538)의 차이가 여기서 온다.

- M2: posterior z가 action loss gradient를 직접 받음 → 행동 예측에 필요한 정보를 충분히 담은 z (action-sufficient z)
- M5~M7: detach 때문에 행동 정보를 담지 못한 z (action-blind z) → decoder가 z에 의존할 이유가 없음

### 병목 2: decoder가 z를 회피할 수 있다 (concat 문제)

현재 action flow는 `input_proj(cat([x_t, t_emb, context, z]))`로 z를 한 번 섞은 후 residual block을 돈다. `input_proj`에서 z 축 weight를 작게 만들면 decoder는 z를 사실상 무시할 수 있다. 병목 1로 z가 action에 약하면, decoder 입장에서 z를 무시하는 것이 locally optimal하다.

### 두 병목의 직렬 관계

```
병목 1 (detach) → z가 action에 약함
                        ↓
         decoder가 z에 의존할 이유 없음
                        ↓
         병목 2 (concat) → decoder가 z 회피
                        ↓
         z_shuffle_gap 낮음, sampling_diversity 감소
```

**하나만 고치면 다시 무너진다.** FiLM만 넣으면 "action-weak z를 더 강하게 주입"하는 꼴이 된다.

### detach가 두 군데인데 성격이 다르다

```python
# [A] action branch detach — 1차 수술 대상
action_cond = cat([context, z_star.detach()])   ← 이것만 제거

# [B] prior target detach — 건드리면 moving target 문제
loss_prior = flow_matching_loss(prior_flow, z_star.detach(), planner)  ← 유지
```

[A]와 [B]를 같이 제거하는 것은 다른 실험이고 더 위험하다. [A]만 먼저.
...
| 순위 | 아이디어 | 시점 | 이유 |
|:----:|---------|:----:|------|
| **0** | **Action detach 제거 (Ablation A)** | **지금 당장** | **병목 1 해결. 한 줄 변경. 이게 안 되면 나머지 다 무의미** |
| 1 | **FiLM z-modulation (Ablation B)** | Ablation A 성공 후 | 병목 2 해결. A 없이 단독으로는 반쪽 |
| 2 | **Classifier-Free Guidance (z dropout, Ablation C)** | Ablation B 와 함께 | z dependency 학습 신호 명시화 |
| 3 | **Soft VQ / Prototype** | Ablation A~C 검증 후 (M9-1) | z 공간 구조화, collapse 방지 |
| 4 | **MoE (z → expert routing)** | M9 후 (M10) | VQ 성공 시 자연스러운 확장 |
| 5 | **World Model (predictive z)** | 중기 | semantic loss 강화, 병목 1 대안 경로 |
| 6 | **Hard VQ** | M9 후 (M9-2) | discrete binding 완성, prior 재설계 필요 |
| 7 | **Information Bottleneck** | 후기 | 이론적으로 제일 강하지만 아키텍처 전반 변경 |
| 8 | **Causal Representation** | 측정 방법으로 이미 사용 중 | 실험 철학 |
| 9 | **Option Learning / Hierarchical** | 후기 | 방향 맞지만 현재 문제 해결 후 |
| 10 | **Slot Attention** | 후기 | 과설계, 현재 단계 아님 |
...
- **Ablation A(action detach 제거) 없이 넣으면 반쪽**: z 자체가 action에 약하면(= z를 action loss로 학습 안 함), null_z와의 차이도 클 수 없음 → "불량 재료를 더 명시적으로 비교하는 꼴"

### 우선순위: ★★★★★
Ablation A + B(FiLM)와 함께 넣어야 제대로 작동. Ablation A → B+C 순서.
...
continuous z를 유지하되, 미리 정해 둔 "대표 벡터" 모음(codebook)의 각 prototype 쪽으로 z를 끌어당기는 손실 — 이걸 commitment loss라고 한다 — 을 추가.
...
- 코드북 선택(argmin) 연산은 미분 불가인데, 역방향 학습 신호(gradient)를 그냥 통과시켜 우회하는 기법(straight-through) 덕분에 gradient 흐름 유지
- codebook이 task prototype을 자연스럽게 학습할 가능성
- 코드북 벡터 일부가 한 번도 선택되지 않아 방치되는 문제(dead code)가 Hard VQ보다 완화됨

### 단점
- z를 prototype 쪽으로 당기는 힘의 세기를 조절하는 계수(commitment weight λ) 튜닝 필요
...
```
z → gating network (어떤 expert를 얼마나 쓸지 비율을 결정하는 작은 네트워크) → expert weights (K개)
...
### 장점
- z가 routing signal이 되는 순간, 구조적으로 반드시 있어야만 동작하는 존재(mandatory)가 됨
...
- 특정 expert만 계속 선택되고 나머지가 방치되지 않도록 골고루 사용하게 강제하는 장치(load balancing) 필요
...
```
현재 문제: z optional (z가 없어도 action 품질 거의 안 변함)
    │
    ├─ [1단계] 병목 1 제거: z가 action loss를 직접 받게 한다
    │   └─ Ablation A: action_detach=False (코드 한 줄)
    │
    ├─ [2단계] 병목 2 제거: decoder가 구조적으로 z를 무시 못 하게 한다
    │   ├─ Ablation B: FiLM (z → γ,β → 각 residual block 변조)
    │   └─ Ablation C: CFG z-dropout (p=0.1, z 없는 조건도 학습)
    │
    ├─ [3단계] z 공간 구조화 (Ablation A~C 검증 후)
    │   ├─ M9-1: Soft VQ (commitment loss, collapse 방지)
    │   └─ M9-2: Hard VQ (discrete binding, prior 재설계)
    │               │
    ...
    │
    └─ 측정/철학
        ├─ Causal Representation (z intervention test, z_drop_test.py)
...
### 지금 당장 (진단)
1. **z_drop test** — M7 ckpt_80에 `scripts/z_drop_test.py` 실행 → decoder가 z를 실제로 무시하는지 수치로 확인
   ```
...
### 단기 (병목 제거 ablation)
2. **Ablation A** — `configs/vlm_paligemma_ablation_a.yaml`에서 `loss.action_detach: false` 설정 후 학습
   - 성공 기준: z_shuffle_gap > 0.3, prior_posterior_gap > 0.1
3. **Ablation B+C** — Ablation A 성공 확인 후 FiLM + CFG z-dropout(p=0.1) 동시 적용
   - 성공 기준: z_shuffle_gap > 0.5, prior_posterior_gap > 0.3

### 중기 (공간 구조화)
4. **M9-1 (Soft VQ)** — Ablation A~C 검증 후
5. **World Model 강화** (semantic loss → proprio prediction 확장)

### 장기
6. **MoE** — VQ 방향 검증 후
7. **Hard VQ** — Soft VQ 성공 후
