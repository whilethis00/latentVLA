...
# ── 헤더 텍스트 ──────────────────────────────────────────────
last_ep = epochs[-1]
fig.text(0.50, 0.955, "LatentVLA — Stage 1 Full Run (SFP 100ep)", ha="center",
         fontsize=18, fontweight="bold", color=TEXT_COLOR)
fig.text(0.50, 0.915,
         f"Exp: StochFlowPrior (M4)  ·  Stage 1 (PaliGemma frozen)  ·  "
         f"LIBERO-Object  ·  {last_ep}/100 epoch",
         ha="center", fontsize=10, color="#555e7a")
fig.text(0.50, 0.890, "2026-04-01 ~",
...
ax2.set_ylabel(f"Loss (Ep {last_ep})", color=TEXT_COLOR, fontsize=8)
...
rows = [
    ...
    ["prior_posterior_gap", f"{final['val/prior_posterior_gap']:.4f}", "higher = z useful", "△ declining, monitor"],
    ["best_of_1",           f"{final['val/best_of_1']:.4f}",           "lower is better",  "OK"],
    ["best_of_5",           f"{final['val/best_of_5']:.4f}",           "lower is better",  f"✓ vs best_of_1"],
    ["future_cosine_sim",   f"{final['val/future_cosine_sim']:.4f}",   "closer to 1",      "✓ ≈ 0.996"],
    ["z_shuffle_gap",       f"{final['val/z_shuffle_gap']:.4f}",       "higher = z relied on", "△ small, monitor"],
]
col_labels = ["Metric", f"Value (Ep {final_ep})", "Criterion", "Assessment"]
...
ax5.set_title(f"Final Validation Metrics Summary  (Epoch {final_ep})",
