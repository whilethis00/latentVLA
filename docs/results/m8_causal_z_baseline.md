...
따라서 M9는 posterior `z` content separation과 content-level counterfactual binding으로 시작한다. Prior-action co-training은 posterior content delta가 오른 뒤에도 prior content delta가 낮을 때 M10으로 진행한다.
...
```bash
# M9: Posterior z content separation + content-level counterfactual binding
# 목적: z가 content plan으로 분리되고 decoder가 그 content를 읽도록 만들기
