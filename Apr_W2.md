...
### 2026-04-15 업데이트: M6 InfoNCE 학습 완료

- 실행 경로: `outputs/runs/vlm_sfp_infonce/`
- 현재 확인 가능한 것은 `train_log.jsonl` 기준 validation 추이까지이며, 오프라인 eval은 아직 없음
- 최고 `z_shuffle_gap`: **0.136956 (epoch 5)**
- 마지막 validation `z_shuffle_gap`: **0.004115 (epoch 20)**
- 마지막 validation `future_cosine_sim`: **0.976565**
- 마지막 validation `action_mse_prior`: **0.849226**

**현재 판단:** InfoNCE는 초반에는 효과가 있었지만 후반에 collapse를 막지 못했다. 즉, "방향은 맞지만 현 설정만으로는 안정적인 해결책이 아니다."
...
| M7 | z-Contrastive (InfoNCE) | 구현 및 1차 학습 완료, 불안정 |
| M8 | Distill + Contrastive | 최종 모델 |

De-risking 관점에서 Distillation 먼저, Best paper 관점에서 Contrastive가 핵심 기여였지만, 실제 1차 InfoNCE 결과는 early gain 후 collapse였다. 따라서 다음 단계는 `M6 z-Distillation`을 예정대로 우선 검증하고, InfoNCE는 sweep/early-stop이 필요한 보조 축으로 보는 편이 타당하다.
...
- 최고 gap 0.137은 "방향성 있음" 신호다.
- 하지만 마지막 gap 0.004는 성공 기준 미달이다.
- 따라서 이번 런은 "Contrastive 단독 해법의 1차 실패"로 기록하는 편이 정확하다.
